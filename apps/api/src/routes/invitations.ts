...
r.get('/invitation-codes/:id', requireScopeGroup('invitationCodesRead'), requireRole('admin'), async (c) => {
  const db = c.get('db');
...
r.post(
  ...
  async (c) => {
    ...
    const [inserted] = await db
      ...
      .returning();
    if (!inserted) throw new ApiException(500, ERROR_CODES.INTERNAL_ERROR, 'Failed to create invitation code');
...
r.post('/invitation-codes/:id/deactivate', requireScopeGroup('invitationCodesWrite'), requireRole('admin'), async (c) => {
  const auth = c.get('auth');
  const db = c.get('db');
  const id = requireParam(c, 'id');
  const [updated] = await db
